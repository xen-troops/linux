				    ...
				    struct vsp1_dl_body *dlb)
{
	...
	if (pipe->output->write_back == WB_STAT_CATP_SET) {
		vsp1_wpf_write(wpf, dlb, VI6_WPF_DSTM_ADDR_Y, wpf->buf_addr[0]);
		if (format->num_planes > 1)
			vsp1_wpf_write(wpf, dlb, VI6_WPF_DSTM_ADDR_C0, wpf->buf_addr[1]);
		if (format->num_planes > 2)
			vsp1_wpf_write(wpf, dlb, VI6_WPF_DSTM_ADDR_C1, wpf->buf_addr[2]);
	}
