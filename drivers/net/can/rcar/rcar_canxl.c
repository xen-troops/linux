...
/* The R-Car CAN XL controller can operate in two mode
 * - CAN-XL mode (default)
 * - CAN-FD mode
 *
...
/* Index 0 =
 * TX_PQ_START_ADD + 32(max descriptor in PQ Queue)*8(element in TX descriptor)*4(byte) = H'400
 * Index 1 =
 * Index 0 + 1023(max descriptor in FQ Queue)*4(element in RX descriptor)*4(byte) = H'3FF0
 */
					 ...
					 struct net_device *ndev)
{
	...
	if (!gpriv->xlmode)
		if (dlc)
			pay_load_size = ((can_fd_dlc2len(dlc) - 1) / 4) + 1;
		else
			pay_load_size = 0;
	else
	...
	if (gpriv->xlmode || (!gpriv->xlmode && can_fd_dlc2len(dlc) > 4))
		ele1 |= CANXL_BIT_PLSRC(1);
	else if (!gpriv->xlmode && can_fd_dlc2len(dlc) <= 4)
	...
	can_put_echo_skb(skb, ndev, priv->tx_head % RCANXL_FIFO_DEPTH, 0);
				    ...
				    u32 fcan_freq)
{
	...
	netif_napi_add_weight(ndev, &priv->napi, rcar_canxl_rx_poll,
			      RCANXL_NAPI_WEIGHT);
