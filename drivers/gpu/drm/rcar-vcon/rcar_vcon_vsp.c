...
/*
 * rcar_vcon_vsp.h  --  R-Car Video Converter VSP-Based Compositor
 *
 * Copyright (C) 2023-2034 Renesas Electronics Corporation
 *
 */
			 ...
			 struct sg_table sg_tables[3])
{
	...
	for (i = 0; i < fb->format->num_planes; ++i) {
		...
		if (gem->sgt) {
			...
			ret = sg_alloc_table(sgt, gem->sgt->orig_nents, GFP_KERNEL);
			if (ret)
				goto fail;
			...
			ret = dma_get_sgtable(rvcon->dev, sgt, gem->vaddr,
...
static int rcar_vcon_vsp_plane_atomic_check(struct drm_plane *plane, struct drm_atomic_state *state)
{
	struct drm_plane_state *new_plane_state = drm_atomic_get_new_plane_state(state, plane);
	struct rcar_vcon_vsp_plane_state *rstate = to_rcar_vsp_plane_state(new_plane_state);

	return __rcar_vcon_plane_atomic_check(plane, new_plane_state, &rstate->format);
}
