				...
				bool is_mem, u32 reg, u32 *data)
{
	...
	val = is_mem ? OPCODE_MEM_READ32 : OPCODE_CONF_READ32;
	val |= SRCID_PROTO_STACK0_ACCESS;

	iowrite32(val, ucie->base + APB_BRIDGE_CTL0);

	val = is_phy ? DSTID_PHY_STACK_ACCESS : DSTID_PROTO_STACK_ACCESS;
	val |= reg;

	iowrite32(val, ucie->base + APB_BRIDGE_CTL1);

	ret = readl_poll_timeout(ucie->base + APB_BRIDGE_STS1, val,
				 (val & COMPL_STATUS) == COMPL_SUCCESS, 1000, 1000000);
				  ...
				  bool is_mem, u32 reg, u32 data)
{
	...
	val = is_mem ? OPCODE_MEM_WRITE32 : OPCODE_CONF_WRITE32;
	val |= SRCID_PROTO_STACK0_ACCESS;
...
{
	rcar_ucie_conf_write32(ucie, (rcar_ucie_conf_read32(ucie, reg) & ~mask) | val, reg);
}

void rcar_ucie_phy_modify32(struct rcar_ucie *ucie, u32 reg, u32 mask, u32 val)
{
	rcar_ucie_phy_write32(ucie, (rcar_ucie_phy_read32(ucie, reg) & ~mask) | val, reg);
}
